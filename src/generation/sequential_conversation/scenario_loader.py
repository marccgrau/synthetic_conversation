...
def load_scenario_data() -> Dict[str, Any]:
    ...
    """
    # Load personal data
    personal_data: Dict[str, Any] = load_yaml("config/personal_data.yaml")

    # Randomly sample the bank, service agent name, and customer name
    selected_bank: str = "Migros Bank"  # random.choice(personal_data["bank_names"])
    ...
    media_types: list[Dict[str, Any]] = load_yaml("config/media_type.yaml")["media_type"]
    ...
    service_agent_components: Dict[str, Any] = load_yaml("config/service_agents.yaml")["service_agent_profile"]
    service_characteristic: str = random.choice(service_agent_components["characteristics"])["description"]
    service_style: Dict[str, str] = random.choice(service_agent_components["conversational_styles"])
    service_emotion: Dict[str, str] = random.choice(service_agent_components["emotional_statuses"])
    service_experience: str = random.choice(service_agent_components["experience"])["description"]
    service_goal: str = random.choice(service_agent_components["goals"])["description"]

    # Load and sample components for Customer Agent
    customer_agent_components: Dict[str, Any] = load_yaml("config/customer_agents.yaml")["customer_agent_profile"]
    customer_characteristic: str = random.choice(customer_agent_components["characteristics"])["description"]
    customer_style: Dict[str, str] = random.choice(customer_agent_components["conversational_styles"])
    customer_emotion: Dict[str, str] = random.choice(customer_agent_components["emotional_statuses"])
    customer_experience: str = random.choice(customer_agent_components["experience"])["description"]
    customer_goal: str = random.choice(customer_agent_components["goals"])["description"]
